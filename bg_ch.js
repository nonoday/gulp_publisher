/**
 * node bg_ch.js <css폴더경로>
 * Background Hex to CSS 변수 변환 스크립트
 * 
 * @description
 * 스크립트 내부의 colorData 배열에서 hex와 ch 매핑을 사용하여,
 * CSS 폴더 내의 모든 CSS 파일에서 background 관련 속성(background, background-color 등)의 
 ...
 * node bg_ch.js html/solid2/page/input-f/css
 * 
 * // input-f 폴더 전체의 CSS 파일 변환 (하위 폴더 포함)
 * node bg_ch.js html/solid2/page/input-f
 * 
 ...
 * - background, background-color 속성에서만 변환됩니다.
...
// CSS 파일에서 background 관련 속성 내의 hex 값을 var(--ch-토큰)으로 변경
function replaceHexInCss(cssContent, colorMap) {
    ...
    colorMap.forEach((ch, hex) => {
        ...
        modifiedContent = modifiedContent.replace(regex, (match, offset, fullText) => {
            // background 관련 속성에서만 교체
            if (isBackgroundProperty(fullText, offset)) {
                localCount += 1;
                return `var(--${ch})`;
            }
            return match;
...
function main() {
    ...
    console.log('🚀 Background Hex to CSS 변수 변환 스크립트 시작\n');
