...
// border 또는 border-color 속성에서만 컬러 값 추출 여부 확인
function isColorProperty(content, position) {
    ...
    // "border" 또는 "border-color" 속성 매칭
    // 패턴: 시작/공백/줄바꿈/중괄호 뒤에 "border" 또는 "border-color"가 오고, 그 뒤에 공백과 콜론/등호
    // 예: "border:", " border:", "\nborder:", "{border:", ";border:"
    // 예: "border-color:", " border-color:", "\nborder-color:", "{border-color:", ";border-color:"
    
    // "border" 또는 "border-color" 속성 매칭
    // 패턴: (시작 또는 공백/중괄호/세미콜론) + "border" 또는 "border-color" + 공백 + (콜론 또는 등호)
    const borderPattern = /(?:^|[\s{;])border(?:-color)?\s*[:=]/;
...
// Hex 컬러 값 추출 (border 또는 border-color 속성만)
function extractHexColors(content) {
    ...
    while ((match = hexPattern.exec(cleanContent)) !== null) {
        ...
        // border 또는 border-color 속성에서만 추출
        if (isColorProperty(cleanContent, position)) {
            const rgb = hexToRgb(hex);
            if (rgb) {
                // 실제 속성 이름 확인
                const startPos = Math.max(0, position - 300);
                const beforeText = cleanContent.substring(startPos, position).toLowerCase();
                const lastSeparator = Math.max(
                    beforeText.lastIndexOf(';'),
                    beforeText.lastIndexOf('}'),
                    beforeText.lastIndexOf('\n')
                );
                const propertyText = beforeText.substring(Math.max(0, lastSeparator));
                const isBorderColor = /border-color\s*[:=]/.test(propertyText);
                
                colors.push({
                    value: hex.toLowerCase(),
                    rgb: rgb,
                    type: 'hex',
                    property: isBorderColor ? 'border-color' : 'border'
                });
            }
        }
    }
...
// RGBA 컬러 값 추출 (border 또는 border-color 속성만)
    ...
    while ((match = rgbaPattern.exec(cleanContent)) !== null) {
        ...
        if (isColorProperty(cleanContent, position)) {
            ...
            if (parsed) {
                // 실제 속성 이름 확인
                const startPos = Math.max(0, position - 300);
                const beforeText = content.substring(startPos, position).toLowerCase();
                const lastSeparator = Math.max(
                    beforeText.lastIndexOf(';'),
                    beforeText.lastIndexOf('}'),
                    beforeText.lastIndexOf('\n')
    ...
    const jsonData = {
        ...
        hexColorGroups: hexGroups.map((group, index) => ({
            ...
            colors: group.map(color => ({
                value: color.value,
                rgb: color.rgb,
                property: color.property || 'border'
            }))
        })),
...
<head>
    ...
    <title>CSS Border/Border-Color 속성 색상 추출 결과</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>🎨 CSS Border/Border-Color 속성 색상 추출 결과</h1>
        <div class="summary">
...
function main() {
    ...
    if (args.length < 1) {
        console.log('사용법: node border_color.js <css파일경로 또는 css폴더경로> [출력파일경로]');
        console.log('\n예시:');
        console.log('  node border_color.js html/solid2/page/input-f/css/style.css');
        console.log('  node border_color.js html/solid2/page/input-f/css');
        console.log('  node border_color.js html/solid2/page/input-f/css colors.html');
        process.exit(1);
    }
    ...
    console.log('🚀 CSS Border/Border-Color 속성 색상 추출 스크립트 시작\n');
