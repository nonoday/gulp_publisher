...
function isColorProperty(content, position) {
    ...
    // "-color" 형태가 있으면 제외 (예: background-color, border-color 등)
    if (/[a-z-]+-color\s*[:=]/.test(propertyText)) {
        return false;
    }
    
    // 정확히 "color" 속성만 매칭 (시작/공백/중괄호/세미콜론 뒤에 color, 그 뒤에 콜론 또는 등호)
...
// CSS 파일에서 color 속성 내의 hex 값을 var(--ch-토큰)으로 변경
function replaceHexInCss(cssContent, colorMap) {
    ...
    colorMap.forEach((ch, hex) => {
        ...
        modifiedContent = modifiedContent.replace(regex, (match, offset, fullText) => {
            // color 속성에서만 교체
            if (isColorProperty(fullText, offset)) {
                localCount += 1;
                return `var(--${ch})`;
            }
            return match;
